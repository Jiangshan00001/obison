...
int bison_file_io::read_stream(std::istream &fin)
{
    ...
    while(std::getline(fin, oneline))
    {
        ...
        {
            //针对 SS :S的情况
            if((ret1[1].size()>1)&&((ret1[1][0]==':')||(ret1[1][0]=='|')))
            {
                std::string sp;
                sp.assign(ret1[1].begin(), ret1[1].begin()+1);
                ret1[1].erase(ret1[1].begin());
                ret1.insert(ret1.begin()+1,sp);
            }
        }
...
int bison_file_io::out_y(std::string file_name, int with_code)
{
    ...
    {
        if((i%5==0)&&((i==0) || (i!=m_tokens.size()-1)))
        {
            fout<<"\n%token ";
        }
        fout<<" "<<m_tokens[i]<<" ";
...
int bison_file_io::generate_bison_table()
{
    /// 利用符号优先级来解决冲突
    ...
    for(unsigned i=0;i<m_tokens.size();++i)
    {
        terms.push_back(m_tokens[i]);
    }
    for(unsigned i=0;i<m_left.size();++i)
    {
        terms.push_back(m_left[i]);
    }
    for(unsigned i=0;i<m_right.size();++i)
    {
        terms.push_back(m_right[i]);
    }
    for(unsigned i=0;i<m_rules.size();++i)
    {
        for(unsigned j=1;j<m_rules[i].size();j++)
        {
            for(unsigned k=0;k<m_rules[i][j].size();k++)
            {
                std::string rule_tk = m_rules[i][j][k];
                if ((rule_tk.size()==3) &&(rule_tk[0]=='\'')&&(rule_tk[2]=='\''))
                {
                    terms.push_back(rule_tk);
                }
            }
        }
    }
    ...
    for(unsigned i=0;i<m_rules.size();++i)
    {
        for(unsigned j=0;j<m_rules[i].size();j++)
        {
            ...
            {
                std::string rule_tk = m_rules[i][j][k];
                if (std::find(terms.begin(), terms.end(),rule_tk)==terms.end())
                {
    ...
    nterms.assign(nterms_set.begin(), nterms_set.end());
